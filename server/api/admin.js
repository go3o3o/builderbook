...
router.post("/books/sync-one-chapter", async (req, res) => {
  ...
  try {
    const sync = fork(
      dev
        ? "./server/api/sync-one-inside-fork.js"
        : "./compiled/server/api/sync-one-inside-fork.js"
    );
...
router.post("/books/sync-all-chapters", async (req, res) => {
  ...
  try {
    const sync = fork(
      dev
        ? "./server/api/sync-all-inside-fork.js"
        : "./compiled/server/api/sync-all-inside-fork.js"
    );
    const userGithubToken = user.githubAccessToken;
...
router.get("/github/repo-files", async (req, res) => {
  ...
  try {
    ...
    const files = mainFolder.data.filter(
